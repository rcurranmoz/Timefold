...
nonisolated final class SharedMemoriesManager: Sendable {
    ...
    /// Save the day's photo for the widget. Returns whether it actually landed —
    /// the caller stamps "done for today" off this, so a silent failure must not
    /// look like success.
    @discardableResult
    func saveWidgetThumbnail(from asset: PHAsset) -> Bool {
        guard let url = containerURL?.appendingPathComponent("widgetPhoto.jpg") else { return false }
        ...
        // Without this, a sample that lives only in iCloud yields no image at
        // all on an "Optimise iPhone Storage" library.
